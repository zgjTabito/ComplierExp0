...
entry:
    ...
    %ptr_f = alloca ptr, align 8     ; 申请 ptr_f 空间 (使用 ptr 类型)
    %ptr_i = alloca ptr, align 8     ; 申请 ptr_i 空间 (使用 ptr 类型)

    ; 设置初始值
    store i32 1, i32* %f, align 4    ; f = 1
    store i32 2, i32* %i, align 4    ; i = 2
    store ptr %f, ptr %ptr_f, align 8 ; ptr_f = &f
    store ptr %i, ptr %ptr_i, align 8 ; ptr_i = &i

    ; 指针操作：*ptr_f = f
    %load_ptr_f = load ptr, ptr %ptr_f, align 8
    %val_f = load i32, i32* %f, align 4
    store i32 %val_f, i32* %load_ptr_f, align 4

    ; 指针操作：*ptr_i = i
    %load_ptr_i = load ptr, ptr %ptr_i, align 8
